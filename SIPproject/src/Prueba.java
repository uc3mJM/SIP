...
*  También muestra como el API corta aquellos campos que no se
...
public static void main(String[] args) {
		...
		System.out.println("Ahora generamos el INVITE por API");
		System.out.println("");
		inv = new InviteMessage();
		inv.setDestination("sip:mario@uc3m.es");
		inv.addVia("1.1.1.1:5060");
		inv.setMaxForwards(70);
		inv.setToName("Mario");
		inv.setToUri("sip:mario@uc3m.es");
		inv.setFromName("Javier");
		inv.setFromUri("sip:javier@uc3m.es");
		Random rand = new Random();
		int min = 1;
		int max = 10000;
		int randomNum = rand.nextInt((max - min) + 1) + min;
		String callId = Integer.toString(randomNum);
		inv.setCallId(callId);
		int cont=1;
		inv.setcSeqNumber(Integer.toString(cont));
		cont++;
		inv.setcSeqStr("INVITE");
		inv.setContact("1.1.1.1:5060");
		...
		inv.setContentType("Application/SDP");
		inv.setSdp(sdp);

		inv.setContentLength(sdp.toStringMessage().length());
		System.out.println("Imprimiendo mensaje INVITE generado por API");
		System.out.println(inv.toStringMessage());

		// Vamos a generar otros mensajes para probar el API
